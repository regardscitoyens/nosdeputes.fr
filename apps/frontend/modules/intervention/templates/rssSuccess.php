...
$feed->setLink(myTools::getProtocol().'://'.$_SERVER['HTTP_HOST'].url_for('@search_interventions_mots?search='.$mots));
...
foreach($query->execute() as $i)
{
  ...
  $item->setLink(myTools::getProtocol().'://'.$_SERVER['HTTP_HOST'].$i->getLink());
