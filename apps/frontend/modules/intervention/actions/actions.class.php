...
class interventionActions extends sfActions
{
  ...
  private function getSectionId(sfWebRequest $request) {
    ...
    if ($section_id != "") {
      if (preg_match('/[a-z]/', $section_id)) {
        $section_id = Doctrine::getTable('Section')->findOneByIdDossierInstitution(strtolower($section_id));
  ...
  public function executeSeanceAPI(sfWebRequest $request) {
    ...
    foreach($this->interventions as $int) {
      ...
      $i['source'] = $int['source'];
      $i['url_nossenateurs'] = myTools::url_forAPI('@interventions_seance?seance='.$int['seance_id'])."#inter_".$int['md5'];
      $i['url_nossenateurs_api'] = myTools::url_forAPI("@api_document?class=Intervention&id=".$int['id']."&format=".$request->getParameter('format'));
      $i['id'] = $int['id'];
