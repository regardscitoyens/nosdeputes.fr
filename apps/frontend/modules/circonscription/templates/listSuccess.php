...
<?php include_partial('circonscription/mapDepartement', array('circo'=>'full', 'height'=>600, 'width'=>546)); ?>
...
foreach ($iters as $iter1 => $iter2) {
 ...
 foreach($circos as $num => $circo) {
  ...
  echo '<p title="'.strtoupper($num).'" class="dept dep_map" id="dep'.strtoupper($num).'">'.link_to($circo, '@list_parlementaires_departement?departement='.preg_replace('/ /', '_', $circo)).'</p>';
  if ($ct == $iter2)
    break; 
 }
} ?>
