...
foreach ($seances as $seance) {
  ...
  echo '<li><a href="'.url_for('@interventions_seance?seance='.$seance->id).'">'.preg_replace('/:/', 'H', $seance->getMoment())."</a>";
  if ($type == "Hémicycle") {
    echo '<ul>';
    foreach ($seance->getTableMatiere() as $section) {
      if ($section['section_id'] == $section['id'] && !(preg_match('/(ordre\sdu\sjour|suspension\sde\séance)/i', $section['titre'])))
        echo '<li><a href="'.url_for('@interventions_seance?seance='.$seance->id).'#table_'.$section['id'].'">'.$section['titre'].'</a></li>';
    }
    echo '</ul>';
  }
  echo '</li>';
