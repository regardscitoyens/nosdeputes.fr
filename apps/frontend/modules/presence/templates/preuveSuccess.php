...
<ul><?php foreach($preuves as $preuve) { ?>
  <li><?php if ($preuve->type == 'compte-rendu') echo 'Liste des présences au bas du ';
       if ($preuve->type != 'jo') echo link_to('Compte-rendu de séance', $preuve['source']);
       else {
         if (preg_match('/(\d{2}\/\d{2}\/\d{4})$/', $preuve->source, $match)) echo '<a href="http://www.journal-officiel.gouv.fr/users.php?date_jo='.$match[1].'">'.$preuve->source.'</a>';
