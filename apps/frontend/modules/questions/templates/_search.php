...
<?php $parlementaire = $question->getParlementaire(); ?>
  <h2><?php echo link_to($parlementaire->nom.'&nbsp;: '.$question->getTitre(), '@question?id='.$question->id); ?></h2>
    </div>
  <div class="texte_question"><?php
  if (!isset($nophoto))
    echo '<a href="'.url_for('@question?id='.$question->id).'" class="intervenant"><img width="50" height="70" alt="'.$parlementaire->nom.'" src="'.url_for('@resized_photo_parlementaire?height=64&slug='.$parlementaire->slug).'" /></a>';
