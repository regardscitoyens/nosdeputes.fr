...
$feed->setLink(myTools::getProtocol().'://'.$_SERVER['HTTP_HOST'].url_for('@search_questions_ecrites_mots?search='.$mots));
...
foreach($query->execute() as $q)
{
  ...
  $item->setLink(myTools::getProtocol().'://'.$_SERVER['HTTP_HOST'].$q->getLink());
