...
class sectionComponents extends sfComponents
{
  public function executeSimplifions() {
    $this->lois = Doctrine_Query::create()->select('l.texteloi_id, l.titre, l.nb_commentaires')->from('TitreLoi l')->where('l.chapitre IS NULL')->andWhere('l.section IS NULL')->orderBy('l.date DESC')->fetchArray();
  }
