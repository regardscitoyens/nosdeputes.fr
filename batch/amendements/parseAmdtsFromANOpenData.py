...
def cleanAuteurs(auteurs, h):
    ...
    auteurs = auteurs.replace(u"groupe de l'", u"groupe ")
    auteurs = auteurs.replace(u"de la Gauche démocrate et républicaine", u"GDR")
    auteurs = auteurs.replace(u"socialiste, républicain et citoyen", u"SRC")
    auteurs = auteurs.replace(u"Socialiste, républicain et citoyen", u"SRC")
    auteurs = auteurs.replace(u"Socialiste, écologiste et républicain", u"SER")
    auteurs = auteurs.replace(u"radical, républicain, démocrate et progressiste", u"RRDP")
    auteurs = auteurs.replace(u"Union des démocrates et indépendants", u"UDI")
    auteurs = auteurs.replace(u"Union pour un mouvement populaire", u"UMP")
    auteurs = auteurs.replace(u"Union pour un Mouvement Populaire", u"UMP")
    auteurs = auteurs.replace(u"commissaiires", u"commissaires")
    auteurs = auteurs.replace(u"commissaires-membres", u"commissaires")
    auteurs = auteurs.replace(u"commissaires membres", u"commissaires")
    auteurs = auteurs.replace(u"commissaires des finances", u"commissaires aux finances")
    auteurs = auteurs.replace(u"aux finances membres du groupe SRC", u"SRC aux Finances")
    auteurs = auteurs.replace(u"commissaires SRC", u"commissaires du groupe SRC")
