...
$string =~ s/\|(\W+)\|/$1/g;
$string =~ s/<\/?sup>//ig;
$string =~ s/<p>\|((?:<a name.*?<\/a>)?Article (?:unique|liminaire|\d+e?r?)[^<]*?)\s*\|\s*\/?(.*?)\s*\/?\s*<\/p>/<p>\/$1 $2\/<\/p>/gi;
...
$string =~ s/<p>(<a name.*?<\/a>)?(Su(?:r le rapport|ivant l'avis) d[^,]*,\s*)?(La commission(?: d\S+(?: \w\w+)*?|, après[^,]*avis[^,]*,)?)[\s\/|]+((?:en vient|émet|accept|donne un avis|procède au|adopt|rejet+)[eé,]*)[\s\/|]*(.*?)<\/p>/<p>\/$1$2$3 $4 $5\/<\/p>/gi;
...
foreach $line (split /\n/, $string)
{
  ...
  if ($prez && $line =~ /<\/?t(able|d|h|r)/) {
    ...
    if (($prez && $line =~ /^\|?(Informations relatives à la Commission|Présences en réunion)/i) || $line =~ /^\W*Membres présents/) {
	  ...
	  } elsif (!($line =~ /^\|(?:&#\d+;|–)?\s*(?:Puis de |Marges de |de |Ateliers? |Table ronde|Premiers? échange|En conséquence|Dispositions|Audition|Organisation|Présentation|Nomination|Commission|Accords?|Anciens|[co]*Présidence|Titre|Chapitre|Section|Après|Avant|Articles?|[^|]*pro(jet|proposition) de (loi|résolution))/i) &&
	...
	if (!$found && !$finished && $line !~ /^\s*M(mes?|[e\.])\s+[^\.:]*(interroge|question|soulève| été nommé|avait assuré|ayant )/) {
      ...
      if ($urlvideo =~ /assemblee-nationale.*\/video\.([^.]+)\./) {
        ...
        while ($sommairevid =~ s/<chapter[^>]*label="\s*([^"]+)\s*"[^>]*>//) {
          ...
          if ($element =~ /^M(?:\.|me)\s+([^,]+),\s+(.*)$/ || $element =~ /^((?:Col|Gal|Dr)[. ]+[^,]+),\s+(.*)$/) {
            ...
            checkout();
          } elsif ($element =~ /^M(?:\.|me)\s+(.*)$/ || $element =~ /^((?:Col|Gal|Dr)[. ]+.*)$/) {
            checkout();
            $intervenant = setIntervenant($1);
            $intervention = $nointer;
            checkout();
