...
foreach $line (split /\n/, $string)
{
  ...
  if ($prez && $line =~ /<\/?t(able|d|h|r)/) {
	...
	if (!$found && !$finished && $line !~ /^\s*M(mes?|[e\.]|adame|onsieur)\s+([^\.:]*(interroge|, pour le rapport|est également|la parole|propose|a p(ubli|os)é|a mené|est nommé|convié|souhaite|répond|question|soulève|empêché|faire part| été nommé|avait assuré|, je vous |, merci |ayant )|[^:]*présentent)/) {
    ...
    if ($line =~ /(https?.*?(assnat\.fr|videos?\.assemblee-nationale\.(fr|tv)|assemblee-nationale\.tv)\/[^\s"<>]*)([\s"<>]|\.$|$)/ && ($line !~ /commission-elargie/ || $source =~ /commissions_elargies/)) {
      ...
      if ($urlvideo =~ /assemblee-nationale.*\/video\.([^.]+)/) {
        ...
        while ($sommairevid =~ s/<chapter[^>]*label="\s*([^"]+)\s*"[^>]*>//) {
          ...
          $element =~ s/^M\.(\S+)/M. \1/;
          $element =~ s/(\S+)\s*\(\s*/\1 (/;
          if ($element =~ /^M(?:\.|me)?\s+([^,]+),\s*(.*)$/ || $element =~ /^((?:(?:Col|Général|Capitaine|Major|Ga?l|S[.\s]+E(?:[.\s]+M)?|Son Excellence|[PD]r)[. ]+)+[^,]+),\s*(.*)$/) {
            checkout();
            $intervenant = setFonction($2, $1);
