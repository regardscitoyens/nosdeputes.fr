...
def html2json(s):
    ...
    for p in p_tags:
        ...
        b = p.find('b')
        if (b):
            if (b.get_text().find('M.') == 0 or b.get_text().find('Mme') == 0 or b.get_text().find('Monsieur') == 0 or b.get_text().find('Madame') == 0 or b.get_text().find('Me ') == 0 or (b.get_text().find(' (') and b.get_text()[-2:] == ').')):
                new_intervention()
