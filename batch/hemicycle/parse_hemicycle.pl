...
$string =~ s/(<i>\([^<).]+\.) (<\/i>)/\1)\2 /g;
$string =~ s/(\(([^)]*?amendement[^)]*?(adopt|rejet)|Nouveaux|M[mes.]+ [^)]* (protes[ent]+|brandi|coupe le micro|applaudi[sen]*t)|Applaudissement|Protestation|Approbation|Exclamation|Vif|Vive|Quelque|M..?mes? mouve|(Sou)?Rires|[^)]*?bancs d[esu]+ groupe)[^)]*\))/<i>\1<\/i>/ig;
