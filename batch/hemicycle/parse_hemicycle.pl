...
$string =~ s/(<i>\([^<).]+\.) (<\/i>)/\1)\2 /g;
$string =~ s/(\(([^)]*?a(rticle|mendement)[^)]*?(adopt|rejet|retir)|Brouha|Il est procédé au scrutin|Nouveaux|M[mes.]+ [^)]* (protes[ent]+|brandi|s'exclame|soupire|coupe le micro|applaudi[sen]*t)|Applaudissement|La séance[^)]*est (suspendue|reprise|ouverte|levée)|Protestation|Approbation|Exclamation|Vif|Vive|Quelque|M..?mes? mouve|(Sou)?Rires|[^)]*?bancs d[esu]+ groupe)[^)]*\))/<i>\1<\/i>/ig;
