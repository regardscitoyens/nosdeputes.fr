...
$string =~ s/(<i>\([^<).]+\.) (<\/i>)/\1)\2 /g;
$string =~ s/(\(([^)]*?a(rticle|mendement)[^)]*?(adopt|rejet|retir)|Il est procédé au scrutin|Nouveaux|M[mes.]+ [^)]* (protes[ent]+|brandi|s'exclame|coupe le micro|applaudi[sen]*t)|Applaudissement|La séance[^)]*est (suspendue|reprise|ouverte|levée)|Protestation|Approbation|Exclamation|Vif|Vive|Quelque|M..?mes? mouve|(Sou)?Rires|[^)]*?bancs d[esu]+ groupe)[^)]*\))/<i>\1<\/i>/ig;
...
#    $line =~ s/<\/?p>//g;
    ...
    if ($line =~ /\<[p]/i) {
        ...
        foreach $line (split /##BR##/, $line) {
            ...
            }elsif ($line =~ s/^\s*\|\s*(M[^\|\/\:]+)[\|\/\:]// || ($line !~ /^\s*M[\.Mmle]+[^.]+ (veut|est |d..?nonce|expliqu)/ && $line =~ s/^\s*(M[\.Mmle]+(\s+([dl][eaus'\s]+)*[^\.:\s]{2,}){1,4})[\.\:]//)) {
                if ($line) {
                    checkout();
                    $majIntervenant = 1;
