    ...
    if e.nodeName == 'g':
        for path in e.getElementsByTagName('path'):
            ...
            for pointset in points:
                paths.append([
                                path.getElementsByTagName('title')[0].firstChild.data
                                + " - " +
                                path.getElementsByTagName('desc')[0].firstChild.data,
                                pointset, 
...
base = sys.argv[1].replace('.svg','')
out = []
out.append('<img class="carte_departement" src="/images/circonscriptions/'+base+'.png" width="'+
str(int(x))+'"height="'+str(int(y))+'"usemap="#'+base+'" /> <map name="'+base+'">');

for g in parsed_groups:
    for path in parsed_groups[g]:
        out.append('<area href="circonscription?search=%s" title="%s" shape="poly" coords="%s"></area>' %
            (path[2], path[0], ', '.join([("%d,%d" % (p[0]*width_ratio, p[1]*height_ratio)) for p in path[1]])))

out.append('</map>')

outfile = open(base+'.html', 'w')
