...
class tagSeanceTask extends sfBaseTask
{
  ...
  protected function wordize($interventions, $excludeS = 0, $minsize = 1) {
    ...
    foreach($interventions as $i) {
      $i = preg_replace('/\x91/', '', $i);
      $i = preg_replace('/<\/?[a-z][^>]+>/i', '', $i);
      $i = preg_replace('/\([^\)]+\)/', '', $i);
